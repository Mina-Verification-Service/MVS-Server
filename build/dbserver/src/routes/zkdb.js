...
async function findUser(userPubKey) {
    const zkDB = await startZkDB();
...
async function addUser(userPubKey, session) {
    const zkDB = await startZkDB();
    const userRecord = zkDB.findOne("userAddress", userPubKey);
    if (!userRecord.isEmpty()) {
...
export async function getDBRootHandler(ctx) {
    try {
...
export async function getUserHandler(ctx) {
    try {
...
export async function addUserHandler(ctx) {
    try {
        let { userPubKey } = ctx.params;
